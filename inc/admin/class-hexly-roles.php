...
class HexlyRoles {
  ...
  public static $version = 1;
	...
	private static $option = 'hexly_roles_version';
  ...
  function add_roles() {
    $opt = get_option( HexlyRoles::$option );
    if (empty($opt) || $opt < HexlyRoles::$version) {
      $this->add_role('hexly_member', 'Hexly Member', ['hexly_member' ]);

			update_option(HexlyRoles::$option, HexlyRoles::$version);
			Hexly::info("Updated hexly roles from $opt to " . HexlyRoles::$version);
    }
  }
  ...
  function add_role($role, $label, $capabilities = []){
    ...
    if (empty($existing)) {
      ...
      Hexly::info('Created role: ' . $role);
    }else{
      Hexly::info('Skipping role as it already exists: ' . $role);
    }
  }
}
